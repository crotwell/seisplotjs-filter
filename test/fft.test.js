...
test("FFT", () => {
  return Promise.all([readSac("./test/data/IU.HRV.__.BHE.SAC"),
                      readSac("./test/data/IU.HRV.__.BHE_fft.sac.am"),
                      readSac("./test/data/IU.HRV.__.BHE_fft.sac.ph")])
  .then ( result => {
      let sac = result[0];
      let sacAmp = result[1];
      ...
      if (false) {
        saveDataPromise = readDataView("./test/data/IU.HRV.__.BHE_fft.sac.am").then(dataView => {
          let inSac = parseSac(dataView);
          expect(bagAmPh.amp.length).toBe(inSac.npts);
          return Promise.all([
              writeSac(replaceYData(dataView, bagAmPh.amp), "./test/data/IU.HRV.__.BHE_fft.bag.am"),
              writeSac(replaceYData(dataView, bagAmPh.phase), "./test/data/IU.HRV.__.BHE_fft.bag.ph")
            ]);
        });
      }
